...
exports.ToolbarButton = function ToolbarButton(options) {
  var unloaders = [],
   ...
   function createUniqueButtonIDAndLabel(){
      if(options.id && options.label){
        if(!prefs.isSet(TBBLibraryPref+"."+options.id)){
          prefs.set(TBBLibraryPref+"."+options.id, false);
          return;
        } else {
          options.id = null;
          options.label = null;
      ...
      if(!options.id){
        options.id = (self.name ? self.name : self.id)+"_btn";
      }
      if(!options.label){
        options.label = (self.name ? self.name : null);
      }
  }
  ...
  function createButtonDOM(doc){
    ...
    if(options.disabled) tbb.setAttribute('disabled', options.disabled);
    if(options.panel)
  ...
  function saveButtonToAllWindows(){
    ...
    for each (var window in winUtils.windows(null, { includePrivate: options.privateWindow })){
    ...
    windows.on("open", function(window){
        ...
        saveButtonToWindow(winUtils.getMostRecentBrowserWindow());
    });
  }
  function saveButtonToWindow(window){
    ...
    var unloadFunc = function() {
      ...
      }, options.id, options.privateWindow);
  ...
  function clearPersistOnOldToolbar(){
    var thisWindow = winUtils.getMostRecentBrowserWindow();
  ...
  function afterCustomization(){
    var doc = winUtils.getMostRecentBrowserWindow().document;
    var toolbarElem = toolbarbuttonExists(doc, options.id);
  ...
  return {
    ...
    get disabled() options.disabled,
    set disabled(value) {
      options.disabled = value;
      getToolbarButtons(function(tbb) {
        tbb.setAttribute('disabled', value);
      }, options.id, options.privateWindow);
      return value;
    },
    setIcon: setIcon,
...
function getToolbarButtons(callback, id, privateWindow) {
  let buttons = [];
  for each (var window in winUtils.windows(null, { includePrivate: privateWindow })){
